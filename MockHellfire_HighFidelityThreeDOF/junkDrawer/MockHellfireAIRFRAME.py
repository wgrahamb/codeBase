	...
	def dynamics(self):
		...
		while GO:
			...
			if self.FLAG == 0:
				...
				EDOTDOT = (OMEGA_AF ** 2) * (self.FIN_DEFLECTION_DEGREES - self.E - 2 * ZETA_AF * self.EDOT / OMEGA_AF) # DERIVATIVE

				self.E += self.EDOT * self.TIME_STEP # STATE
				self.EDOT += EDOTDOT * self.TIME_STEP # STATE AND DERIVATIVE

				ETERM = K3 * (self.E + TA * self.EDOT)
				self.THETA_DOT_DEG = ETERM# STATE AND DERIVATIVE
				self.THETA_DOT = np.radians(self.THETA_DOT_DEG)

				NORMAL_ACCEL = K1 * (self.E - (EDOTDOT / (OMEGA_Z ** 2)))# * self.G
				self.SPECIFIC_FORCE = npa([0.0, NORMAL_ACCEL])
				LOCAL_G = npa([0.0, -1.0 * self.G])
				BODY_G = self.BODY_TO_RANGE_AND_ALTITUDE @ LOCAL_G
				self.SPECIFIC_FORCE += BODY_G
			...
			self.POSITION += DELTA_POS # STATE

			DELTA_VEL = self.ACCELERATION * self.TIME_STEP
			self.VELOCITY += DELTA_VEL # STATE AND DERIVATIVE
			self.BODY_VELOCITY = self.BODY_TO_RANGE_AND_ALTITUDE @ self.VELOCITY

			self.ALPHA += self.ALPHA_DOT * self.TIME_STEP # Radians. # STATE
			self.ALPHA_DOT = self.THETA_DOT - (self.SPECIFIC_FORCE[1] / self.SPEED) # Radians per second. # DERIVATIVE
			self.ALPHA_DEG = np.degrees(self.ALPHA)
			self.ALPHA_DOT_DEG = np.degrees(self.ALPHA_DOT)

			if self.FLAG == 0:

				self.THETA += self.THETA_DOT * self.TIME_STEP # Radians. # STATE
				self.THETA_DOT += THETA_DOT_DOT * self.TIME_STEP # Radians per second. # STATE AND DERIVATIVE
...
if __name__ == "__main__":
	...
	x = AirframeSimulation(
		...
		INITIAL_MISSILE_ALTITUDE=1000.0,
		INITIAL_HRZ_VEL=300.0,
		INITIAL_VRT_VEL=300.0
	)
	x.dynamics()
	x = AirframeSimulation(
		FLAG=1,
