...
void control(Missile &missile)
{

	{
		...
		if (missile.machSpeed > 0.01) {

			double DNA = missile.CNA * (q * REFERENCE_AREA / missile.mass); // METERS PER SECOND^2
			double DND = missile.CND * (q * REFERENCE_AREA / missile.mass); // METERS PER SECOND^2
			double DMA = missile.CMA * (q * REFERENCE_AREA * REFERENCE_DIAMETER / missile.transverseMomentOfInertia); // PER SECOND^2
			double DMD = missile.CMD * (q * REFERENCE_AREA * REFERENCE_DIAMETER / missile.transverseMomentOfInertia); // PER SECOND^2
			double DMQ = missile.CMQ * (REFERENCE_DIAMETER / (2 * missile.speed)) * (q * REFERENCE_AREA * REFERENCE_DIAMETER / missile.transverseMomentOfInertia); // PER SECOND
			double DLP = missile.CLP * (REFERENCE_DIAMETER / (2 * missile.speed)) * (q * REFERENCE_AREA * REFERENCE_DIAMETER / missile.axialMomentOfInertia); // PER SECOND
			double DLD = missile.CLD * (q * REFERENCE_AREA * REFERENCE_DIAMETER / missile.axialMomentOfInertia); // PER SECOND^2

			// ROLL
			double GKP = (2 * ROLL_CONTROL_WN * ROLL_CONTROL_ZETA + DLP) / DLD;
			double GKPHI = ROLL_CONTROL_WN * ROLL_CONTROL_WN / DLD;
			double EPHI = GKPHI * (0.0 - missile.ENUEulerAngles[0]);
