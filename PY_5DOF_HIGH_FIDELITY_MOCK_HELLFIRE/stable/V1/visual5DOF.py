...
# Setup.
f1 = r"PY_5DOF_HIGH_FIDELITY_MOCK_HELLFIRE/output/MOCK_HELLFIRE5DOF.txt"
viewFile = f1
...
if scale:
	...
	yMin = min(list(df.iloc[startIndex:stopIndex]["POS_0Y"]))
	yMax = max(list(df.iloc[startIndex:stopIndex]["POS_0Y"]))
	zMin = min(list(df.iloc[startIndex:stopIndex]["POS_0Z"]))
	zMax = max(list(df.iloc[startIndex:stopIndex]["POS_0Z"]))
...
trajectory.plot(
	...
	df.iloc[startIndex:stopIndex]["POS_0Z"],
	color=colors.pop(0)
)
...
pitch.plot(df.iloc[startIndex:stopIndex]["TOF"], df.iloc[startIndex:stopIndex]["THT_0"] * RAD_TO_DEG, label="THETA - DEG", color=colors.pop(0))
pitch.plot(df.iloc[startIndex:stopIndex]["TOF"], df.iloc[startIndex:stopIndex]["THTDOT_0"] * RAD_TO_DEG, label="THETA DOT - DEG", color=colors.pop(0))
...
yaw.plot(df.iloc[startIndex:stopIndex]["TOF"], df.iloc[startIndex:stopIndex]["PSI_0"] * RAD_TO_DEG, label="PSI - DEG", color=colors.pop(0))
yaw.plot(df.iloc[startIndex:stopIndex]["TOF"], df.iloc[startIndex:stopIndex]["PSIDOT_0"] * RAD_TO_DEG, label="PSI DOT - DEG", color=colors.pop(0))
