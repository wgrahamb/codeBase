	...
	def dynamics(self):
		...
		while GO:
			...
			if self.FLAG == 0:
				...
				self.NORMAL_ACCEL = K1 * (self.E - (self.EDOTDOT / (OMEGA_Z ** 2))) * self.G # NEEDS TO BE M/S^2
				self.SPECIFIC_FORCE = npa([(self.THRUST / self.MASS), self.NORMAL_ACCEL]) # NEEDS TO BE M/S^2
				LOCAL_G = npa([0.0, -1.0 * self.G])
				BODY_G = self.BODY_TO_RANGE_AND_ALTITUDE @ LOCAL_G
				self.SPECIFIC_FORCE += BODY_G
				self.ACCELERATION = self.SPECIFIC_FORCE @ self.BODY_TO_RANGE_AND_ALTITUDE # DERIVATIVE
