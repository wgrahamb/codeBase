...
)
{
	if (dataPacket->mslMach > 0.6) {
		...
		double DNA = CNA * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea / dataPacket->mslMass); // METERS PER SECOND^2
		double DMA = CMA * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslTMoi); // PER SECOND^2
		double DMD = CMD * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslTMoi); // PER SECOND^2
		double DMQ = CMQ * (dataPacket->mslRefDiam / (2 * dataPacket->mslSpeed)) * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslTMoi); // PER SECOND
		double DLP = CLP * (dataPacket->mslRefDiam / (2 * dataPacket->mslSpeed)) * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslAMoi); // PER SECOND
		double DLD = CLD * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslAMoi); // PER SECOND^2
		...
		// PITCH
		double zzdNew = normCommand - dataPacket->mslBodyAcc[2];
		...
		// YAW
		double yydNew = dataPacket->mslBodyAcc[1] - sideCommand;
		...
		double DNA = CNA * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea / dataPacket->mslMass); // METERS PER SECOND^2
		double DND = CND * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea / dataPacket->mslMass); // METERS PER SECOND^2
		double DMA = CMA * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslTMoi); // PER SECOND^2
		double DMD = CMD * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslTMoi); // PER SECOND^2
		double DMQ = CMQ * (dataPacket->mslRefDiam / (2 * dataPacket->mslSpeed)) * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslTMoi); // PER SECOND
		double DLP = CLP * (dataPacket->mslRefDiam / (2 * dataPacket->mslSpeed)) * (dataPacket->mslDynamicPressure * dataPacket->mslRefArea * dataPacket->mslRefDiam / dataPacket->mslAMoi); // PER SECOND
