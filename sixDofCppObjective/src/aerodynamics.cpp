...
aerodynamics::aerodynamics(mslDataPacket *dataPacket, std::string inPutDataFile, double alpha_prime_max)
{
	std::cout << "AERODYNAMICS INITIATED" << std::endl;
	alphaPrimeMax = alpha_prime_max;
	formatDataTables(inPutDataFile);
...
void aerodynamics::update(mslDataPacket *dataPacket)
{
	aeroBallisticAngles(dataPacket);
	aeroDynamicCoefficients(dataPacket);
	aeroDynamicReferenceValues(dataPacket);
...
void aerodynamics::aeroBallisticAngles(mslDataPacket *dataPacket)
{
	...
	double pitchDeflAeroFrame = dataPacket->pitchFinDefl * cosPhiPrime - dataPacket->yawFinDefl * sinPhiPrime;
	pitchDeflAeroDeg = radToDeg * pitchDeflAeroFrame;
	double yawDeflAeroFrame = dataPacket->pitchFinDefl * sinPhiPrime + dataPacket->yawFinDefl * cosPhiPrime;
	yawDeflAeroDeg = radToDeg * yawDeflAeroFrame;
	rollDeflDeg = radToDeg * dataPacket->rollFinDefl;
...
void aerodynamics::aeroDynamicCoefficients(mslDataPacket *dataPacket)
{
	...
	if (dataPacket->mslTof <= dataPacket->mslBurnOut) {
		CAOFF = 0.0;
	}
	else {
	...
	double CNAERO = CNAEROREF - CYAERO * (dataPacket->mslLaunchCg - dataPacket->mslCgFromNose) / dataPacket->mslRefDiam;
	...
	double CMAERO = CMAEROREF - CZAERO * (dataPacket->mslLaunchCg - dataPacket->mslCgFromNose) / dataPacket->mslRefDiam;
	...
	if (availAccel < 0) {
		dataPacket->mslMaxAccel = 1;
	}
	else if (availAccel > dataPacket->mslMaxAccelAllow) {
		dataPacket->mslMaxAccel = dataPacket->mslMaxAccelAllow;
	}
	else {
		dataPacket->mslMaxAccel = availAccel;
	}

}

void aerodynamics::aeroDynamicReferenceValues(mslDataPacket *dataPacket)
{
	...
	CMA = ((CLM0MAX - CLM0MIN) / (alphaPrimeDegPlusThree - alphaPrimeDegMinusThree) - (CNA / radToDeg) * (dataPacket->mslLaunchCg - dataPacket->mslCgFromNose) / dataPacket->mslRefDiam) * radToDeg;
	CND = CNDQ * radToDeg;
	CMD = CLMDQ * radToDeg;
	CMQ = CLMQ * radToDeg;
	CLP = CLLP * radToDeg;
